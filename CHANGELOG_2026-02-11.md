...
### 6. Crash Fix (Auto-Reconnect)
- **Problem:** Crash when attempting to auto-reconnect due to a method name mismatch (`connect_to_server` vs `handle_connect`).
- **Solution:** Updated `_attempt_reconnect` in `main_window.py` to call the correct method `handle_connect`.

### 7. Crash Fix (Memory Warning)
- **Problem:** Crash when clicking "Save Logs Now" in the memory warning dialog due to a method name mismatch (`save_current_logs` vs `save_logs_to_file`).
- **Solution:** Updated `_show_memory_warning` in `main_window.py` to call the correct method `save_logs_to_file`.

### 8. Crash Fix (Stuck "Refreshing" State)
- **Problem:** Application getting stuck in "Refreshing..." state if a network error (like `getaddrinfo failed`) occurs during pod refresh.
- **Solution:** Updated `_on_error` handler to ensure the "Refresh" button is always re-enabled even if an error occurs.
- **Bonus:** Added specific handling for DNS errors to show a helpful message about VPN/Internet connection instead of a raw socket error.
