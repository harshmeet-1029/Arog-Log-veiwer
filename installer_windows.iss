...
[Files]
Source: "dist\ArgoLogViewer-Installer\*"; DestDir: "{app}"; Flags: ignoreversion recursesubdirs createallsubdirs
...
begin
  ...
  // Force close any running instances of ArgoLogViewer
  Exec('taskkill', '/F /IM ArgoLogViewer.exe', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  Exec('taskkill', '/F /IM ArgoLogViewer-Installer.exe', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  Exec('taskkill', '/F /IM ArgoLogViewer-Portable.exe', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  
  // Small delay to ensure processes are fully closed
  Sleep(1000);
